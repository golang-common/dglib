...
func (d *Txn) DelNode(obj interface{}) (*api.Response, error) {
	...
	uid, _, err = d.getUidAndDType(structVal)
	if err != nil {
		return nil, err
	}
	if uid == "" {
	...
	for i := 0; i < structVal.NumField(); i++ {
		...
		tagList := strings.Split(field.Tag.Get(TagDb), ",")
		if len(tagList) == 0 {
			continue
		}
		pred := tagList[0]
...
func (d *Txn) nquad(obj interface{}, action string, omitempty ...bool) ([]*api.NQuad, []string, error) {
	...
	uid, dtype, err := d.getUidAndDType(structVal)
	if err != nil {
		return nil, nil, err
	}
	...
	if action != actionDelete {
		r = append(r, &api.NQuad{
			...
			ObjectValue: &api.Value{Val: &api.Value_StrVal{StrVal: dtype}},
		})
	}
	// 遍历解析结构体
	for i := 0; i < structVal.NumField(); i++ {
...
// getUidAndDType 从传入的结构体对象中获取dgraph.type字段的值
func (d *Txn) getUidAndDType(value reflect.Value) (string, string, error) {
	tp := value.Type()
	uidTp, ok := tp.FieldByName("Uid")
	if !ok {
		return "", "", errors.New(fmt.Sprintf("uid field not found in type [%s]", value.Type()))
	}
	dtype := uidTp.Tag.Get("dtype")
	if dtype == "" {
		return "", "", errors.New(fmt.Sprintf("dtype tag in uid field not found in type [%s]", value.Type()))
	}
	uidVal := value.FieldByName("Uid")
	if uidVal.IsZero() {
		return "", dtype, nil
	}
	v, ok := uidVal.Interface().(string)
	if !ok {
		return "", "", errors.New("error uid type, need string")
	}
	return v, dtype, nil
}
...
func (d *Txn) parseObjNquad(subject, pred string, obj interface{}) (*api.NQuad, error) {
	...
	case int:
		r.ObjectValue = &api.Value{Val: &api.Value_IntVal{IntVal: int64(obj.(int))}}
	case float64:
	...
	if reflect.TypeOf(obj).Kind() == reflect.Struct && !reflect.ValueOf(obj).IsZero() {
		val := reflect.ValueOf(obj)
		tp := reflect.TypeOf(obj)
		subUid, _, err := d.getUidAndDType(val)
		if err != nil {
			return nil, err
		}
		if subUid == "" {
		...
		for i := 0; i < val.NumField(); i++ {
			subTag := tp.Field(i).Tag.Get(TagDb)
